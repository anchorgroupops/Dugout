      ...
      <div className="glass-panel" style={{ padding: isMobile ? 'var(--space-lg)' : '1.5rem', marginBottom: '2rem' }}>
        ...
        {standings?.as_of && (
          <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>
            As of {formatDateMMDDYYYY(standings.as_of)}
          </div>
        )}
        ...
        <>
          <div style={{
            ...
            <div style={{
              ...
              letterSpacing: '0.5px', color: 'var(--primary-color)',
            }}>
            ...
            <span style={{
              ...
              {divisionRivals.length}
            </span>
          </div>
          <div style={{
            fontSize: 'var(--text-xs)', color: 'var(--text-muted)',
            marginBottom: '0.75rem', fontStyle: 'italic', paddingLeft: '0.25rem',
          }}>
            Players may be borrowed between these teams
          </div>
          <div className="card-grid" style={{ marginBottom: '1.5rem' }}>
            {divisionRivals.map(renderOpponentCard)}
          </div>
        </>
      )}

      {/* Opponent Scouting — Other Opponents */}
      {otherOpponents.length > 0 && (
        <>
          <div style={{
            display: 'flex', alignItems: 'center', gap: '0.5rem',
            marginBottom: '0.75rem', marginTop: divisionRivals.length > 0 ? '0.5rem' : '1rem',
          }}>
            <div className="section-label section-label--muted" style={{ margin: 0 }}>
              Other Opponents
            </div>
            <span style={{
              fontSize: 'var(--text-xs)', fontWeight: '600', padding: '2px 8px',
              ...
              {otherOpponents.length}
            </span>
          </div>
          <div className="card-grid">
            {otherOpponents.map(renderOpponentCard)}
          </div>
        </>
      )}
    </div>
    ...
    <div style={{ marginTop: '1rem', paddingTop: '0.75rem', borderTop: '1px solid var(--surface-border)' }}>
      {matchup && !matchup.empty && (
        <div style={{ marginBottom: 'var(--space-sm)' }}>
          {matchup.last_played && (
            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>
              Last played: {formatDateMMDDYYYY(matchup.last_played)}
            </div>
          )}
          {matchup.next_game && (
            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>
              Next game: {formatDateMMDDYYYY(matchup.next_game)}
            </div>
          )}
