...
class GameChangerScraper:
    ...
    def _click_tab(self, tab_text: str) -> bool:
        ...
        try:
            # 1. Try GC tab chooser classes — multiple patterns for selector resilience
            for _class_pat in [
                ...
                if loc.count() > 0:
                    try:
                        ...
                        print(f"[GC]   ✓ Clicked '{tab_text}' via {_class_pat}")
                        return True
                    except Exception:
                        continue
            ...
            if tab_text in ["Standard", "Advanced", "Breakdown", "Catching", "Innings Played"]:
                ...
                if dropdown.is_visible():
                    ...
                    # Options may be in labels, role=option, or li elements
                    option = self.page.locator(
                        f"label[title*='{tab_text}'], "
                        f"[role='option']:has-text('{tab_text}'), "
    ...
    def scrape_all_stats(self) -> dict | None:
        ...
        # Wait for stats tab UI — try multiple selector patterns for resilience against GC redesigns
        _STATS_TAB_SELS = [
            ".TabViewChooserItem__tabViewChooserItem",
            "[class*='TabViewChooser']",
            "[class*='tabViewChooser']",
        ...
        for _sel in _STATS_TAB_SELS:
            try:
                print(f"[GC] Waiting for stats UI via: {_sel}")
                self.page.wait_for_selector(_sel, timeout=12000)
                print(f"[GC] ✓ Stats UI detected.")
                _found_stats_ui = True
                break
            except Exception:
                continue
        if not _found_stats_ui:
            print(f"[GC] [WARN] Stats tab UI not found via any selector (URL: {self.page.url}). Waiting 5s and continuing...")
