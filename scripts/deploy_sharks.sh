#!/bin/bash
# MANUAL ONLY — superseded by Watchtower; port updated.
# Watchtower auto-deploys new GHCR images on the Pi (see
# docker-compose.sharks.yml); this script is kept only as a manual
# fallback and is not run by any CI/CD path.
#
# deploy_sharks.sh — Deploy Sharks Dashboard to Raspberry Pi
#
# SAFETY: This script ONLY touches port 3200 via a standalone docker-compose.
...
echo "[2/3] Starting sharks_dashboard container on port 3200..."
cd "$PROJECT_DIR"
...
if curl -s -o /dev/null -w "%{http_code}" http://localhost:3200 | grep -q "200"; then
    echo "      ✅ Dashboard is live at http://localhost:3200"
else
...
echo "Next step: Add Cloudflare tunnel for dugout.joelycannoli.com -> http://192.168.7.222:3200"
